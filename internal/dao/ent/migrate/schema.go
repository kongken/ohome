// Code generated by ent, DO NOT EDIT.

package migrate

import (
	"entgo.io/ent/dialect/sql/schema"
	"entgo.io/ent/schema/field"
)

var (
	// PostsColumns holds the columns for the "posts" table.
	PostsColumns = []*schema.Column{
		{Name: "id", Type: field.TypeString, Unique: true, Size: 40},
		{Name: "author_id", Type: field.TypeString, Size: 40},
		{Name: "content", Type: field.TypeString, Size: 2147483647},
		{Name: "attachments", Type: field.TypeJSON, Nullable: true},
		{Name: "hashtags", Type: field.TypeJSON, Nullable: true},
		{Name: "community_id", Type: field.TypeString, Nullable: true, Size: 64},
		{Name: "event_id", Type: field.TypeString, Nullable: true, Size: 64},
		{Name: "visibility", Type: field.TypeString, Size: 32, Default: "public"},
		{Name: "likes_count", Type: field.TypeInt, Default: 0},
		{Name: "comments_count", Type: field.TypeInt, Default: 0},
		{Name: "shares_count", Type: field.TypeInt, Default: 0},
		{Name: "created_at", Type: field.TypeTime},
		{Name: "updated_at", Type: field.TypeTime},
		{Name: "deleted_at", Type: field.TypeTime, Nullable: true},
	}
	// PostsTable holds the schema information for the "posts" table.
	PostsTable = &schema.Table{
		Name:       "posts",
		Columns:    PostsColumns,
		PrimaryKey: []*schema.Column{PostsColumns[0]},
		Indexes: []*schema.Index{
			{
				Name:    "post_created_at",
				Unique:  false,
				Columns: []*schema.Column{PostsColumns[11]},
			},
			{
				Name:    "post_author_id_created_at",
				Unique:  false,
				Columns: []*schema.Column{PostsColumns[1], PostsColumns[11]},
			},
			{
				Name:    "post_community_id_created_at",
				Unique:  false,
				Columns: []*schema.Column{PostsColumns[5], PostsColumns[11]},
			},
			{
				Name:    "post_visibility_created_at",
				Unique:  false,
				Columns: []*schema.Column{PostsColumns[7], PostsColumns[11]},
			},
		},
	}
	// UsersColumns holds the columns for the "users" table.
	UsersColumns = []*schema.Column{
		{Name: "id", Type: field.TypeString, Unique: true, Size: 40},
		{Name: "username", Type: field.TypeString, Unique: true, Size: 64},
		{Name: "email", Type: field.TypeString, Unique: true, Size: 254},
		{Name: "password_hash", Type: field.TypeString, Size: 255},
		{Name: "display_name", Type: field.TypeString, Nullable: true, Size: 128},
		{Name: "title", Type: field.TypeString, Nullable: true, Size: 128},
		{Name: "bio", Type: field.TypeString, Nullable: true, Size: 2147483647},
		{Name: "avatar_url", Type: field.TypeString, Nullable: true},
		{Name: "cover_url", Type: field.TypeString, Nullable: true},
		{Name: "location", Type: field.TypeString, Nullable: true, Size: 128},
		{Name: "interests", Type: field.TypeJSON, Nullable: true},
		{Name: "email_verified", Type: field.TypeBool, Default: false},
		{Name: "created_at", Type: field.TypeTime},
		{Name: "updated_at", Type: field.TypeTime},
	}
	// UsersTable holds the schema information for the "users" table.
	UsersTable = &schema.Table{
		Name:       "users",
		Columns:    UsersColumns,
		PrimaryKey: []*schema.Column{UsersColumns[0]},
		Indexes: []*schema.Index{
			{
				Name:    "user_created_at",
				Unique:  false,
				Columns: []*schema.Column{UsersColumns[12]},
			},
		},
	}
	// UserFollowingColumns holds the columns for the "user_following" table.
	UserFollowingColumns = []*schema.Column{
		{Name: "user_id", Type: field.TypeString, Size: 40},
		{Name: "follower_id", Type: field.TypeString, Size: 40},
	}
	// UserFollowingTable holds the schema information for the "user_following" table.
	UserFollowingTable = &schema.Table{
		Name:       "user_following",
		Columns:    UserFollowingColumns,
		PrimaryKey: []*schema.Column{UserFollowingColumns[0], UserFollowingColumns[1]},
		ForeignKeys: []*schema.ForeignKey{
			{
				Symbol:     "user_following_user_id",
				Columns:    []*schema.Column{UserFollowingColumns[0]},
				RefColumns: []*schema.Column{UsersColumns[0]},
				OnDelete:   schema.Cascade,
			},
			{
				Symbol:     "user_following_follower_id",
				Columns:    []*schema.Column{UserFollowingColumns[1]},
				RefColumns: []*schema.Column{UsersColumns[0]},
				OnDelete:   schema.Cascade,
			},
		},
	}
	// Tables holds all the tables in the schema.
	Tables = []*schema.Table{
		PostsTable,
		UsersTable,
		UserFollowingTable,
	}
)

func init() {
	UserFollowingTable.ForeignKeys[0].RefTable = UsersTable
	UserFollowingTable.ForeignKeys[1].RefTable = UsersTable
}
